...
export const ODPCPage: React.FC = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const loadProgress = async () => {
      try {
        const res = await progressService.getProgress(courseData?.id || '1');
        ...
        const key = `ipc_completed_${user?.id || 'guest'}_${courseData?.id || 1}`;
  ...
  const markLessonCompleted = async (mIdx: number, lIdx: number) => {
    ...
    try {
      await progressService.saveProgress(courseData?.id || '1', lessonKey, true);
    } catch (err) {
  ...
  const toggleLessonCompleted = async (mIdx: number, lIdx: number) => {
    ...
    try {
      await progressService.saveProgress(courseData?.id || '1', lessonKey, !isCompleted);
    } catch (err) {
    ...
    const loadCourseData = () => {
      ...
      if (saved) {
        try {
          ...
          const found = list.find((c: any) => c.id === 1 || c.title.toLowerCase().includes('ocpd') || c.title.toLowerCase().includes('datos personales'));
          if (found) {
            setCourseData(found);
          }
        } catch (e) {
        ...
        try {
          ...
          const hasApprovedPayment = res.payments?.some(
            (p: any) => p.status === 'APROBADO' && 
                       ((p.courseTitle || '').toLowerCase() === 'oficial de datos personales' ||
                        (p.courseTitle || '').toLowerCase() === 'certificación oficial en ocpd: protección de datos personales')
          );
  ...
  const modules = [
    { title: "Módulo 1: Introducción a la Protección de Datos", lessons: 4 },
    { title: "Módulo 2: Ley N° 29733 y su Reglamento", lessons: 6 },
    { title: "Módulo 3: RGPD Europeo - Visión Comparada", lessons: 5 },
    { title: "Módulo 4: Principios del Tratamiento de Datos", lessons: 5 },
    { title: "Módulo 5: Derechos del Titular de Datos", lessons: 6 },
    { title: "Módulo 6: Medidas de Seguridad y Técnicas", lessons: 5 },
    { title: "Módulo 7: Transferencia Internacional de Datos", lessons: 4 },
    { title: "Módulo 8: Brechas de Seguridad y Notificación", lessons: 5 },
    { title: "Módulo 9: AUTORIDAD NACIONAL: ANPD", lessons: 4 },
    { title: "Módulo 10: Evaluación de Acreditación OCPD", lessons: 12 },
  ];
      ...
      list = modules.map((m, idx) => {
        ...
        const subModules = Array.from({ length: numLessons }, (_, sIdx) => {
          ...
          if (idx === 1) {
            ...
            title = names[sIdx] || `Lección ${idx + 1}.${sIdx + 1}`;
          } else {
  ...
  const benefits = [
    { icon: <ShieldAlert size={20} />, title: "Ley N° 29733", desc: "Domina completamente la Ley de Protección de Datos Personales del Perú y su reglamento." },
    { icon: <BookOpen size={20} />, title: "RGPD Europeo", desc: "Comprende el Reglamento General de Protección de Datos y aplica sus mejores prácticas." },
    { icon: <Activity size={20} />, title: "Gestión de Brechas", desc: "Aprende a detectar, contener y notificar incidentes de seguridad de datos correctamente." },
    { icon: <FileText size={20} />, title: "Derechos ARCO", desc: "Domina el procedimiento de atención de derechos de Acceso, Rectificación, Cancelación y Oposición." },
    { icon: <MonitorPlay size={20} />, title: "Simulacros ANPD", desc: "Practica con simulacros basados en casos reales resueltos por la Autoridad Nacional de Protección de Datos." },
    { icon: <CheckCircle2 size={20} />, title: "Acreditación OCPD", desc: "Obtén la certificación habilitante como Oficial de Datos Personales reconocida en el mercado peruano." },
  ];

  const testimonios = [
    { text: "El módulo del RGPD comparado con la Ley 29733 es brillante. Me permitió asesorar a un cliente internacional en su expansión al Perú.", author: "Abogada especialista en tecnología" },
    { text: "Aprobé la evaluación OCPD a la primera. El programa prepara muy bien para el examen y sobre todo para la práctica real.", author: "DPO en empresa de telecomunicaciones" },
    { text: "Excelente cobertura del tema de brechas de seguridad y el rol de la ANPD. Conocimiento que hoy aplico diariamente en mi trabajo.", author: "Jefe de Seguridad de la Información" },
    { text: "El mejor programa de protección de datos disponible en el mercado peruano. Equilibrado, actualizado y con casos completamente relevantes.", author: "Gerente de Sistemas en retail" },
  ];
  ...
  const otrosCursos = useMemo(() => {
    ...
    return list
      ...
      .filter((c: any) => {
        ...
        if (c.title.toLowerCase().includes('ocpd') || c.title.toLowerCase().includes('datos personales')) return false;
        return true;
      })
      .map((c: any) => {
          ...
          {!isPurchased ? (
            <>
              ...
              <div className="hacker-course-hero-grid">
                <div>
                  <div style={{ fontSize: '10px', color: 'var(--primary)', textTransform: 'uppercase', letterSpacing: '0.15em', fontWeight: 700, marginBottom: '16px', display: 'inline-block', border: '1px solid var(--primary)', padding: '4px 8px', borderRadius: '4px' }}>CERTIFICACIÓN PROFESIONAL</div>
                  <h2 className="course-hero-title" style={{ textTransform: 'uppercase' }}>
                    {courseData?.title || "OFICIAL DE DATOS PERSONALES CERTIFICADO"}: <br/><span style={{ color: 'var(--primary)' }}>DOMINA LA LEY N° 29733 Y EL RGPD PARA PROTEGER DATOS EN TU ORGANIZACIÓN</span>
                  </h2>
                  <p style={{ fontSize: '14px', color: 'var(--text-muted)', lineHeight: '1.6', marginBottom: '24px' }}>
                    {courseData?.descriptionTop || "Conviértete en Oficial de Datos Personales certificado. Domina la Ley N° 29733, el Reglamento LPDP y el RGPD europeo. Obtén la habilitación profesional para gestionar y proteger datos personales en cualquier organización."}
                  </p>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
                    ...
                    <span style={{ fontSize: '13px', fontWeight: 700 }}>4.8/5 <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(1,200 valoraciones)</span></span>
                  </div>
                  ...
                  {isPending ? (
                    ...
                    <div className="desktop-only-flex" style={{ display: 'flex', gap: '16px' }}>
                      <button onClick={() => setShowPaymentModal(true)} className="btn-primary" style={{ padding: '14px 32px', fontSize: '13px', borderRadius: '9999px', fontWeight: 700 }}>ADQUIRIR AHORA — {courseData?.price || "S/ 750"}</button>
              ...
              <div className="mobile-only-hero-video-cta" style={{ flexDirection: 'column', gap: '20px', marginBottom: '40px' }}>
                ...
                {isPending ? (
                   ...
                   <div className="course-cta-buttons" style={{ width: '100%' }}>
                     <button onClick={() => setShowPaymentModal(true)} className="btn-primary" style={{ justifyContent: 'center', padding: '14px 16px', fontSize: '13px', borderRadius: '9999px', fontWeight: 700 }}>ADQUIRIR AHORA — {courseData?.price || "S/ 750"}</button>
              ...
              <div className="course-stats-grid" style={{ padding: '32px 0', borderTop: '1px solid var(--overlay-light)', borderBottom: '1px solid var(--overlay-light)', marginBottom: '64px' }}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '32px', fontWeight: 800, marginBottom: '4px', color: 'var(--primary)', fontFamily: 'var(--font-title)' }}>842</div>
                  <div style={{ fontSize: '10px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700 }}>ALUMNOS</div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '32px', fontWeight: 800, marginBottom: '4px', color: 'var(--primary)', fontFamily: 'var(--font-title)' }}>10</div>
                  <div style={{ fontSize: '10px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700 }}>MÓDULOS</div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '32px', fontWeight: 800, marginBottom: '4px', color: 'var(--primary)', fontFamily: 'var(--font-title)' }}>56</div>
              ...
              <div style={{ marginBottom: '64px' }}>
                <h3 style={{ fontSize: '24px', fontWeight: 800, marginBottom: '32px', fontFamily: 'var(--font-title)', textTransform: 'uppercase' }}>
                  ¿QUÉ APRENDERÁS <span style={{ color: 'var(--primary)' }}>EN ESTA CERTIFICACIÓN?</span>
                </h3>
              ...
              <div className="course-layout" style={{ marginBottom: '80px' }}>
                ...
                <div>
                  ...
                  <h3 style={{ fontSize: '24px', fontWeight: 800, marginBottom: '32px', fontFamily: 'var(--font-title)', textTransform: 'uppercase' }}>TEMARIO DE LA CERTIFICACIÓN</h3>
                    ...
                    {(() => {
                      ...
                      if (activeMods.length === 0) {
                        activeMods = modules.map((m, idx) => ({
                          ...
                          description: '',
                          subModules: [
                            { id: 1, title: "Marco legal y fundamentos", description: "" },
                            { id: 2, title: "Aplicación práctica y casos", description: "" },
                            { id: 3, title: "Herramientas de implementación", description: "" }
                          ]
                        }));
                      }
                              ...
                              <div style={{ marginTop: '16px' }}>
                                {mod.description ? renderFormattedDescription(mod.description) : (
                                  <p style={{ margin: '8px 0', color: 'var(--text-muted)' }}>
                                    Este módulo contiene {mod.subModules?.length || 0} lecciones sobre protección de datos personales y cumplimiento normativo.
                                  </p>
                                )}
              ...
              <div style={{ padding: '64px 48px', borderRadius: '24px', textAlign: 'center', border: '1px solid var(--overlay-light)', backgroundColor: 'var(--overlay-light)', margin: '0 auto', maxWidth: '800px' }}>
                <h2 style={{ fontSize: '24px', fontWeight: 800, marginBottom: '16px', fontFamily: 'var(--font-title)', textTransform: 'uppercase' }}>
                  ¿LISTO PARA CONVERTIRTE EN <br/><span style={{ color: 'var(--primary)' }}>OFICIAL DE DATOS PERSONALES?</span>
                </h2>
                <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '32px', maxWidth: '600px', margin: '0 auto 32px auto', lineHeight: '1.6' }}>
                  Únete a más de 800 profesionales certificados que ya protegen los datos personales en sus organizaciones.
                </p>
                 {isPending ? (
              ...
              <div className="player-main-grid">
                ...
                <div>
                  ...
                  <p style={{ color: 'var(--text-muted)', fontSize: '14px', lineHeight: '1.6', marginBottom: '32px' }}>
                    {currentLesson?.description || "En esta lección estudiaremos a profundidad los temas planteados en el temario de protección de datos personales y cumplimiento regulatorio."}
                  </p>
                    ...
                    <div style={{ display: 'flex', gap: '16px', marginTop: '8px' }}>
                      <button
                        ...
                        onClick={() => {
                          ...
                          link.download = `Certificado_OCPD_${user?.name?.replace(/\s+/g, '_') || 'Estudiante'}.jpg`;
                          link.target = '_blank';
                          link.click();
                        }}
                        className="btn-primary"
